/**
 * Figma webhook receiver — 다중 화면 배치 모드.
 *
 *   node --env-file=.env server/webhook.mjs
 *
 * FILE_UPDATE / FILE_VERSION_UPDATE 이벤트 → 디바운스 → detect-changes.mjs 로
 * figma-map.json 의 노드 중 바뀐 것만 골라 → 각 노드에 대해
...
async function runBatch() {
  ...
  try {
    ...
    if (code !== 0) {
      log('detect-changes failed — aborting batch')
      return
    }
